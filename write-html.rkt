...
(define (attributes? x)
  (or (null? x) (and (list? x) (list? (car x)))))
...
(module+ test
  (require rackunit)
  (check-equal? (html->string '(hr ())) "<hr>")
  (check-equal? (html->string '(hr)) "<hr>")
  (check-equal? (html->string '(div ())) "<div></div>")
  (check-equal? (html->string '(div)) "<div></div>")
  (check-equal? (html->string '(p "ble<p")) "<p>ble&lt;p</p>")
  (check-equal? (html->string '(a ([href "./foo.html"]) "blap")) "<a href=\"./foo.html\">blap</a>")
  (check-equal? (html->string "ab<cd>ef&gh\"ij") "ab&lt;cd&gt;ef&amp;gh\"ij")
  (check-equal? (html->string '(p ([id "ab<cd>ef&gh\"ij"])))
                "<p id=\"ab&lt;cd&gt;ef&amp;gh&quot;ij\"></p>")
  (check-equal?
   (html->string '(div ()
